    ...
    if ($insert) {
        ...
        if ($invoice) {
            $_SESSION['alertData'] = [
                ...
                'total_transfer' => number_format($invoice['total_transfer'], 0, ',', '.'),
                'bank_tujuan' => $invoice['bank_tujuan'],
                'no_rekening' => $invoice['no_rekening'],
                'nama_pemilik_rekening' => $invoice['nama_pemilik_rekening'],
                'gambar_bank' => !empty($invoice['gambar_bank']) ? "../uploads/bank/{$invoice['gambar_bank']}" : '',
                'redirect' => "paddock.php?id_event=$id_event"
            ];
        }
...
// ==== Mulai HTML setelah semua proses PHP di atas ====
...
<body class="bg-gradient-to-r from-blue-200 via-white to-white min-h-screen">
    <div class="container mx-auto px-4 py-8 max-w-6xl">
        ...
        <form id="paddockForm" action="" method="POST" class="bg-white p-6 rounded-xl shadow-lg space-y-6 border border-gray-100">
            <!-- DATA PENDAFTAR -->
            <h2 class="text-lg text-blue-800 bg-blue-100 px-5 py-2 rounded-full inline-block shadow-sm">DATA PENDAFTAR</h2>
            <input type="text" name="nama_pesanan" placeholder="Nama Pesanan" class="w-full p-3 border rounded-lg focus:ring-2 focus:ring-blue-400 focus:outline-none transition">
            <input type="text" name="nama_tim" placeholder="Nama Tim" class="w-full p-3 border rounded-lg focus:ring-2 focus:ring-blue-400 focus:outline-none transition">
            <input type="text" name="nomor_wa" placeholder="Nomor WhatsApp" class="w-full p-3 border rounded-lg focus:ring-2 focus:ring-blue-400 focus:outline-none transition">

            <!-- SLOT PADDOCK -->
            <h2 class="text-lg text-blue-800 bg-blue-100 px-5 py-2 rounded-full inline-block shadow-sm mt-4">PILIH SLOT PADDOCK</h2>
            ...
            <!-- BUTTON -->
            <button type="submit" class="bg-blue-600 hover:bg-blue-800 text-white font-semibold px-6 py-3 rounded-lg shadow-md transition transform hover:-translate-y-1">
                Submit Pendaftaran
            </button>
        </form>
    </div>

    <!-- Kirim alertData ke JS -->
    <?php if (isset($_SESSION['alertData'])): ?>
        <script>
            window.alertData = <?php echo json_encode($_SESSION['alertData']); ?>;
        </script>
        <?php unset($_SESSION['alertData']); ?>
    <?php endif; ?>

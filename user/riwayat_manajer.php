...
try {
    if ($type === 'manajer') {
        $sql = "
            SELECT m.*, pr.nama_provinsi,
                   GROUP_CONCAT(DISTINCT CONCAT(mk.kelas, ' - ', mk.warna_kendaraan, ' ', mk.tipe_kendaraan) SEPARATOR ', ') AS kelas_kendaraan,
        ...
        $sql = "
            SELECT p.*, pr.nama_provinsi,
                   GROUP_CONCAT(DISTINCT CONCAT(pk.kelas, ' - ', pk.warna_kendaraan, ' ', pk.tipe_kendaraan) SEPARATOR ', ') AS kelas_kendaraan,
                   NULL AS peserta_kelas
            FROM peserta p
        ...
        $sql = "
            SELECT * FROM (
                SELECT m.id_manajer AS id, m.nama_manajer AS nama, pr.nama_provinsi,
                       GROUP_CONCAT(DISTINCT CONCAT(mk.kelas, ' - ', mk.warna_kendaraan, ' ', mk.tipe_kendaraan)) AS kelas_kendaraan,
                       NULL AS peserta_kelas,
                       m.created_at, m.foto_manajer, m.nama_tim, m.email, m.whatsapp, m.voucher
                FROM manajer m
                LEFT JOIN manajer_kelas mk ON mk.manajer_id = m.id_manajer
                LEFT JOIN provinsi pr ON m.id_provinsi = pr.id_provinsi
                WHERE m.id_manajer = (SELECT MAX(id_manajer) FROM manajer)
                GROUP BY m.id_manajer
                UNION
                SELECT p.id_peserta AS id, p.nama_peserta AS nama, pr.nama_provinsi,
                       GROUP_CONCAT(DISTINCT CONCAT(pk.kelas, ' - ', pk.warna_kendaraan, ' ', pk.tipe_kendaraan)) AS kelas_kendaraan,
                       NULL AS peserta_kelas,
                       p.created_at, NULL AS foto_manajer, NULL AS nama_tim, p.email, p.whatsapp, p.voucher
                FROM peserta p
                LEFT JOIN peserta_kelas pk ON pk.peserta_id = p.id_peserta
                LEFT JOIN provinsi pr ON p.id_provinsi = pr.id_provinsi
                WHERE p.id_peserta = (SELECT MAX(id_peserta) FROM peserta)
                    ...
                    <div class="p-4">
                        <div class="flex gap-4">
                            ...
                            <div class="flex-1">
                                ...
                                <?php endif; ?>
                                <span class="inline-block bg-indigo-100/70 text-indigo-600 text-xs font-medium px-2 py-0.5 rounded mt-1">
